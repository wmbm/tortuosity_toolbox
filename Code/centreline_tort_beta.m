...
%Tortuosity Calculations - SUMMARY
%   INPUT
...
SOAM = sum(a)/(sum(b)); 
data_table(2) = real(SOAM);

%---------------------------------------------------------------
% Standard deviation of curvature [SDOC]
%---------------------------------------------------------------
[t,~,~]=frenet(data_xyz(:,1),data_xyz(:,2),data_xyz(:,3));

% Standardised difference between data points
a=[data_xyz(1,1),data_xyz(1,2),data_xyz(1,3)];
b=[data_xyz(1,1),data_xyz(1,2),data_xyz(1,3)];
dS=norm(a-b);

% Curvature
k = norm(diff(t)); 

% Curvature average per unit length
k_m = (dS/d)*sum(k(3:-2));

% Standard deviation of the curvature average per unit length
k_sd = ((dS/d)^0.5)*sqrt(sum((k(3:-2)- k_m).^2));
